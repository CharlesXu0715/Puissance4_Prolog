...
% -------------------------- Added code ----------------------------
% Use C++ code with prolog
:- use_foreign_library(connect4).
...
nextMove('X',X,_, _):- wins('O',X),
		  write('Machine wins!').
nextMove('O',X,_, _):- wins('X',X),
		  write('You win!').
nextMove(_,X,_, _):- full(X),
		write('Draw').
nextMove('X',X,H, L):- repeat, %repeats in case a column is full
		  readColumn2(C, L),
		  append(L, [C], L2),
		  play('X',C,X,X2), !,
		  show(X2),
		  nextMove('O',X2,H, L2).
nextMove('O',X,H, L):- machine('O','X',X,X2,H, C),
                 append(L, [C], L2),
		  show(X2),
		  nextMove('X',X2,H, L2).
...
machine(R,_,T,T2,1, C):- alpha_beta(R,2,T,-200,200,C,_Value,1),
		    play(R,C,T,T2),
		    nl, write('machine: '),
		    associateChar(L,C),
		    write(L), nl.
...
machine(R,_,T,T2,2, C):- alpha_beta(R,2,T,-200,200,C,_Value,2),
		    play(R,C,T,T2),
		    nl, write('machine: '),
		    associateChar(L,C),
		    write(L), nl.
...
% 3 play defensively
machine(R,_,T,T2,3, C):- heuristic_defensive('O',board(T),C),
		    play(R,C,T,T2),
		    nl, write('machine: '),
		    associateChar(L,C),
		    write(L), nl.
...
machine(R,_,T,T2,4, C):- heuristic_aggressive('O',board(T),C),
		    play(R,C,T,T2),
		    nl, write('machine: '),
		    associateChar(L,C),
		    write(L), nl.
...
% else play randomly
machine(R,_,T,T2,_, C):- heuristic_random(C),
		    play(R,C,T,T2),
		    nl, write('machine: '),
		    associateChar(L,C),
		    write(L), nl.
...
heuristic_aggressive(Player,T,C) :- TT=T,
